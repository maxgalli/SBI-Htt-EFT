...
make_gridpack () {
    ...
    #           echo "set cluster_temp_path `echo $RUNHOME`" >> mgconfigscript 
      ...
      # fi
      if [ -e $CARDSDIR/${name}_extramodels.dat ]; then
        echo "Loading local SMEFTsim UFO model (patched version)"
        
        # absolute path to your patched tarball
        SMEFT_UFO_TARBALL="/eos/user/z/zhaom/htautau/samples/SBI-Htt-EFT/models/SMEFTsim_topU3l_MwScheme_UFO_fixed.tar.gz"

        # check that it exists
        if [[ ! -f "$SMEFT_UFO_TARBALL" ]]; then
          echo "ERROR: UFO tarball not found at $SMEFT_UFO_TARBALL" >&2
          exit 1
        fi

        echo "Using local model file $SMEFT_UFO_TARBALL"
        echo "Extracting model into work/models/"

        # ensure we're in the work directory (the script already cd's there earlier)
        mkdir -p models
        cd models

        # extract tarball into this directory
        tar -xavf "$SMEFT_UFO_TARBALL"

        # go back to the work directory
        cd ..

        echo "Finished extracting local SMEFTsim UFO model."
      fi
